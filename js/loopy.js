var Loopy = (function () {
  ...
  var numRows = 6,
      ...
      edges = Array(numRows),
      allEdges = [],
      counts = new Array(numRows),
      corners = new Array(numRows + 1),
      canvas,
      context,
      offset,
      controls;
  ...
  function repaintGrid() {
    ...
    if (looped && twoCount != 0) {
      ...
      while (true) {
        ...
        for (var dir = 0; dir < 4; ++dir) {
          ...
          var neighbor = corners[info.r][info.c];
          if (!neighbor.visited) {
            break;
          }
        }
    ...
    var satisfied = true;
    for (var r = 0; satisfied && r < numRows; ++r) {
      var rowCounts = counts[r];
      for (var c = 0; c < numCols; ++c) {
        var need = rowCounts[c].need;
        if (need != -1 && rowCounts[c].have != need) {
          satisfied = false;
          break;
        }
      }
    }
    var inactiveColor = color.edge.inactive,
        activeColors = color.edge.active,
  ...
  function showCounts(r, c) {
    var have = counts[r][c].have,
        haveDisplay = controls[r][c].display.have;
    haveDisplay.innerHTML = (have == 0 ? '' : '' + have);
    var need = counts[r][c].need,
        needDisplay = controls[r][c].display.need;
  ...
  function load() {
    ...
    context.fillStyle = color.edge.inactive;
    context.fillRect(0, 0, width, height);
    context.fillStyle = color.cell;
    controls = new Array(numRows);
    function makePane() {
    ...
    function makeDisplay() {
      ...
      display.style.fontSize = font.display.size + 'px';
      return display;
    }
    function addMouseHandlers(display) {
      var minusButton = display.buttons.minus,
          ...
          count = counts[position.r][position.c];
    ...
    for (var r = 0; r < numRows; ++r) {
      ...
      for (var c = 0; c < numCols; ++c) {
        ...
        minusButton.style.left = button.left + 'px';
        minusButton.style.top = button.top + 'px';
        minusButton.style.fontSize = font.button.size + 'px';
        plusButton.style.right = button.left + 'px';
        plusButton.style.top = button.top + 'px';
        plusButton.style.fontSize = font.button.size + 'px';
